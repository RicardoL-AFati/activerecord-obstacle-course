...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '12. should return all items except items: 3, 4 & 5' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items = Item.where("id NOT IN (?)", items_not_included.map(&:id))
    # ------------------------------------------------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(orders: [:order_items]).where("order_items.item_id = 7").order("users.name").pluck("distinct users.name")
    # ------------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.where(user_id: user_2).joins(:order_items).where("item_id = 4")
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.joins(:orders).group("items.id").order("items.id")
    # ---------------------------------------------------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    ordered_items_names = Item.joins(:orders).pluck("distinct items.name")
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).group("users.id").select("users.*, COUNT(*) as total_order_count").order("users.name")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:order_items).group("users.id").select("users.*, COUNT(*) as total_item_count").order("users.name desc")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: [:order_items]).group("orders.id").select("users.name AS user_name, orders.id AS order_id, COUNT(*) AS item_count").order("users.name desc")
    # ---------------------------------------------------------------
